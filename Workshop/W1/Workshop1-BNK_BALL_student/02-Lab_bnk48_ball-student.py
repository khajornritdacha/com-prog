...
pg.mixer.music.load(PATH + "/sound.mp3")
pg.mixer.music.play(-1, 0.0)

# ใช้คำสั่ง soundeffect.play() เพื่อเล่นเสียง effect ตอนลูกบอลชนกัน
soundeffect = pg.mixer.Sound(PATH + "/effect.wav")
...
ball1_img = pg.image.load(PATH + "/BNK48/Wee_cc.png").convert_alpha()
...
ball2_img = pg.image.load(PATH + "/BNK48/Cherprang_cc.png").convert_alpha()
...
ball3_img = pg.image.load(PATH + "/BNK48/Orn_cc.png").convert_alpha()
...
while running:
    ...
    if running:
        ...
        font = pg.font.Font(font_name, 80)
        text_surface = font.render("6532155621", True, white)
        text_rect = text_surface.get_rect()
        ...
        ball1_cen = [ball1_rect.left + ball1_radius, ball1_rect.top + ball1_radius]
